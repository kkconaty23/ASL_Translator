...
# ====== Config ======
# Mirror each image/sequence in-memory to reduce handedness bias
AUGMENT_MIRROR = True
...
def extract_84(img_bgr) -> Tuple[Optional[List[float]], Dict[str, Any]]:
    """
    ...
    if not results.multi_hand_landmarks:
        return None, {"detected": 0, "mp_results": results}
...
def originals_and_mirror(img_bgr):
    """Yield the original image and, if enabled, a horizontally flipped variant."""
    yield img_bgr
    if AUGMENT_MIRROR:
        yield cv2.flip(img_bgr, 1)  # horizontal flip
...
# For quick visualization (one per class if available; always from original)
examples_vis: Dict[str, Tuple[np.ndarray, str, int]] = {}  # class_name -> (img_rgb, path, n_hands)
...
for class_name in class_names:
    ...
    if not is_dynamic:
        ...
        for img_path in imgs:
            ...
            # Process original + (optionally) mirrored variants
            for variant_idx, img_bgr in enumerate(originals_and_mirror(img_bgr0)):
                ...
                # keep first vis per class, from the original only
                if class_name not in examples_vis and variant_idx == 0:
                    vis = cv2.cvtColor(img_bgr0, cv2.COLOR_BGR2RGB).copy()
                    res = meta["mp_results"]
        ...
        for seq_dir in seq_dirs:
            ...
            # Accumulate features for original sequence
            seq_feats: List[List[float]] = []
            # Accumulate features for mirrored sequence (if enabled)
            seq_feats_flip: Optional[List[List[float]]] = [] if AUGMENT_MIRROR else None

            for fpath in frames:
                total_imgs_seen += 1
                img_bgr0 = cv2.imread(str(fpath))
                if img_bgr0 is None:
                    continue
                total_imgs_readable += 1

                # Original
                feat84, meta = extract_84(img_bgr0)
                if feat84 is not None:
                    total_imgs_with_hands += 1
                    seq_feats.append(feat84)

                    # keep one visualization per dynamic class, from original only
                ...
                # Mirrored
                if AUGMENT_MIRROR:
                    img_bgr_flip = cv2.flip(img_bgr0, 1)
                    feat84f, _ = extract_84(img_bgr_flip)
                    if feat84f is not None:
                        total_imgs_with_hands += 1
                        assert seq_feats_flip is not None
                        seq_feats_flip.append(feat84f)

            # keep original sequence if non-empty
            if len(seq_feats) > 0:
            ...
            # keep mirrored sequence if enabled and non-empty
            if AUGMENT_MIRROR and seq_feats_flip and len(seq_feats_flip) > 0:
                arrf = np.asarray(seq_feats_flip, dtype=np.float32)
                X_dyn.append(arrf)
                y_dyn.append(class_idx)
                seq_lengths.append(arrf.shape[0])
...
# -------------------------- Visualize examples --------------------------
# Show up to 3 examples in class index order, if available (originals only)
